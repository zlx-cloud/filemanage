...
<body>
	<form class="layui-form" action="">
		...
		<input type="hidden" id="type" name="type" value="${type}"> <input
			type="hidden" id="id" name="id" value="${file.id}">
			...
			<div class="layui-inline">
				...
				<div class="layui-input-block">
					<input type="text" id="serviceName" name="serviceName"
						lay-verify="serviceName" autocomplete="off" placeholder="请输入地址名称"
						class="layui-input form-option" value="${file.serviceName}">
				</div>
			</div>
		</div>
	...
	<script>
		layui.use([ 'layer', 'form' ], function() {
			...
			form.on('submit(submitBtn)', function(data) {
				$.ajax({
					url : '${ctx}/remoteGetFileConfig/edit',
					type : "POST",
					async : false,
					data : {
						id : data.field.id,
